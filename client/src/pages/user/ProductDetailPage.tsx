            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="aspect-w-1 pt-10 aspect-h-1 w-full overflow-hidden rounded-lg border border-luxury-gold/15 bg-[#141414]">
                  <img src={currentProduct.productImage} alt={currentProduct.title} className="w-full lg:h-[500px] object-cover rounded-lg" />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  <h2 className="font-logo text-2xl font-bold text-luxury-cream mb-2">{currentProduct.title}</h2>
                  <p className="text-luxury-cream/70 text-lg leading-relaxed">{currentProduct.description}</p>
                </div>

                <div className="space-y-4">
                  {/* Price */}
                  <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                    <span className="text-lg font-semibold text-luxury-gold">Price:</span>
                    <span className="text-2xl font-bold text-luxury-cream">Rs. {currentProduct.price}</span>
                  </div>

                  {/* Status */}
                  <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                    <span className="text-lg font-semibold text-luxury-gold">Status:</span>
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${currentProduct.status === "available" ? "bg-luxury-gold/15 text-luxury-gold border border-luxury-gold/30" : "bg-red-950/40 text-red-300 border border-red-900/40"}`}>
                  ...
                  {/* Stock */}
                  <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                    <span className="text-lg font-semibold text-luxury-gold">Stock:</span>
                    <span className="text-lg font-medium text-luxury-cream">{currentProduct.stock} units</span>
                  </div>

                  {/* Category */}
                  <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                    <span className="text-lg font-semibold text-luxury-gold">Category:</span>
                    <span className="text-lg font-medium text-luxury-cream capitalize">{currentProduct.category}</span>
                  </div>

                  {/* Quantity */}
                  <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                    <span className="text-lg font-semibold text-luxury-gold">Quantity:</span>
                    <span className="text-lg font-medium text-luxury-cream">{currentProduct.Quantity}</span>
                  </div>

                  {/* Badges */}
                  <div className="flex flex-wrap gap-2 mb-4">
                    {currentProduct.isFeatured && (
                      <span className="px-3 py-1 text-sm font-bold bg-luxury-gold text-luxury-ink rounded-full">⭐ Featured Product</span>
                    )}
                    {currentProduct.isNewArrival && (
                      <span className="px-3 py-1 text-sm border border-luxury-gold text-luxury-gold rounded-full">🆕 New Arrival</span>
                    )}
                    {currentProduct.isOnSale && (
                      <span className="px-3 py-1 text-sm bg-luxury-gold/10 text-luxury-gold border border-luxury-gold/30 rounded-full">🏷️ On Sale</span>
                    )}
                  </div>
                  ...
                  {currentProduct.isOnSale && currentProduct.discountPrice && (
                    <>
                      <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                        <span className="text-lg font-semibold text-luxury-gold">Original Price:</span>
                        <span className="text-lg text-luxury-cream/40 line-through">Rs. {currentProduct.price}</span>
                      </div>
                      <div className="flex items-center justify-between p-4 bg-[#141414] border border-luxury-gold/10 rounded-lg">
                        <span className="text-lg font-semibold text-luxury-gold">Sale Price:</span>
                        <span className="text-2xl font-bold text-luxury-gold">Rs. {currentProduct.discountPrice}</span>
                      </div>
                    </>
                  )}
                      ...
                      return (
                        <>
                          <button
                            ...
                            className={`mt-3 flex items-center justify-center w-12 rounded-md border transition-colors duration-300 disabled:cursor-wait ${isWishlisted ? "bg-red-500 border-red-500 text-white" : "border-luxury-gold/30 text-luxury-cream hover:border-luxury-gold hover:bg-luxury-gold/10 hover:text-luxury-gold"}`}
                          >
                            {isWishlistBusy ? (
                          ...
                          <button
                            ...
                            className={`mt-3 flex items-center justify-center w-12 rounded-md border transition-colors duration-300 disabled:cursor-not-allowed disabled:opacity-50 ${isCompared ? "bg-blue-600 border-blue-600 text-white" : "border-luxury-gold/30 text-luxury-cream hover:border-luxury-gold hover:bg-luxury-gold/10 hover:text-luxury-gold"}`}
                          >
                            {isCompareBusy ? (
                              <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                            ) : (
